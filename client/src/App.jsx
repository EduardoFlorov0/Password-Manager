...
function WindowsMark({ size = 18 }) {
  return (
    <span className="windows-mark" style={{ "--icon-size": `${size}px` }} aria-hidden="true">
      <span />
      <span />
      <span />
      <span />
...
const entryIconOptions = [
  { id: "lock", label: "Катинар", Icon: LockKeyhole },
  { id: "windows", label: "Windows", Icon: WindowsMark },
  { id: "web", label: "Уебсайт", Icon: Globe },
  { id: "mail", label: "Поща", Icon: Mail },
  { id: "card", label: "Карта", Icon: CreditCard },
  { id: "shop", label: "Магазин", Icon: ShoppingCart },
  { id: "games", label: "Игри", Icon: Gamepad2 },
  { id: "work", label: "Работа", Icon: Building2 },
  { id: "security", label: "Сигурност", Icon: ShieldCheck }
];

function getEntryIcon(iconId) {
  return entryIconOptions.find((option) => option.id === iconId) || entryIconOptions[0];
}
...
export default function App() {
  ...
  const hasDraftEntry = [entryForm.service, entryForm.username, entryForm.password, entryForm.notes].some(
  ...
  async function handleSaveEntry(event) {
    ...
    try {
      const encrypted = await encryptVaultEntry(session.encryptionKey, {
        ...
        icon: getEntryIcon(entryForm.icon).id
      });
  ...
  function handleEdit(entry) {
    ...
    setEntryForm({
      ...
      notes: entry.notes || "",
      icon: getEntryIcon(entry.icon).id
    });
  }
  ...
  async function handleCopyPassword(password, entryId) {
    try {
      await navigator.clipboard.writeText(password);
      window.clearTimeout(copyNoticeTimerRef.current);
      setCopiedEntryId(entryId);
      copyNoticeTimerRef.current = window.setTimeout(() => setCopiedEntryId(null), 1800);
      setAuthError("");
    } catch {
      ...
      <section className="workspace">
        <aside className="editor-panel">
          ...
          <form className="entry-form" onSubmit={handleSaveEntry}>
            ...
            <div className="icon-picker" role="group" aria-label="Категория на записа">
              <span>Категория</span>
              <div className="icon-picker-grid">
                {entryIconOptions.map(({ id, label, Icon }) => (
                  <button
                    key={id}
                    type="button"
                    className={entryForm.icon === id ? "icon-choice active" : "icon-choice"}
                    title={label}
                    aria-label={label}
                    aria-pressed={entryForm.icon === id}
                    onClick={() => setEntryForm((current) => ({ ...current, icon: id }))}
                  >
          ...
          <div className="entry-list">
            {filteredEntries.length === 0 ? (
              ...
              filteredEntries.map((entry) => {
                const { label, Icon } = getEntryIcon(entry.icon);
                return (
                <article key={entry.id} className="entry-card">
                  <div className="entry-main">
                    <div className="entry-title-row">
                      <span className="entry-icon" title={label} aria-label={label}>
